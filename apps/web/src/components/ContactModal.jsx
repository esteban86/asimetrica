...
const ContactModal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nombre || !formData.email || !formData.empresa) {
      toast.error("Por favor, completa nombre, correo y empresa.");
      return;
    }
        ...
        <DialogContent className="sm:max-w-[425px] bg-background border-border">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 mt-4">
            <div className="space-y-2">
              ...
              <Input id="nombre" name="nombre" placeholder="Ej. Maya Chen" value={formData.nombre} onChange={handleChange} required disabled={isLoading} className="bg-background text-foreground border-input focus-visible:ring-primary" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="email" className="text-foreground font-medium">Correo electrónico</Label>
              <Input id="email" name="email" type="email" placeholder="maya@empresa.com" value={formData.email} onChange={handleChange} required disabled={isLoading} className="bg-background text-foreground border-input focus-visible:ring-primary" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="empresa" className="text-foreground font-medium">Empresa</Label>
              <Input id="empresa" name="empresa" placeholder="Nombre de tu empresa" value={formData.empresa} onChange={handleChange} required disabled={isLoading} className="bg-background text-foreground border-input focus-visible:ring-primary" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="telefono" name="telefono" type="tel" placeholder="+57 300 000 0000" value={formData.telefono} onChange={handleChange} disabled={isLoading} className="bg-background text-foreground border-input focus-visible:ring-primary" />
            </div>

            <Button type="submit" className="w-full mt-6 bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-200 active:scale-[0.98]" disabled={isLoading}>
